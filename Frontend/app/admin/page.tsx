...
export default function AdminDashboard() {
  ...
  const handleSarchTerm = (e: string) => {
    setSearchTerm(e);
    if (!e.trim()) {
    ...
    const results = allBookings.filter((booking) =>
      booking.name.toLowerCase().includes(lowerValue) ||
      booking.phone.toLowerCase().includes(lowerValue) ||
      booking._id.toLowerCase().includes(lowerValue)
    );

    setFilteredBooking(results);
  }
  ...
  const handleFilter = (status: string) => {
    ...
    setIsDropdown(false);
    if (!status) {
      setFilteredBooking(allBookings); // reset
    } else {
      const isConfirmed = status === "confirmed";
      const results = allBookings.filter(
        (b) => b.isBookingConfirmed === isConfirmed
      );
      setFilteredBooking(results);
    }
  };
  ...
  async function handleConfirmation(bookingId: string) {
    if (!window.confirm("Are you sure you want to confirm this booking?")) return;

    const token = localStorage.getItem("adminToken");

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/update-booking/${bookingId}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
                <div className="relative">
                  ...
                  <input
                    ...
                    value={searchTerm}
                    onChange={(e) => handleSarchTerm(e.target.value)}
                    className="pl-10 pr-4 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>

                  <Button variant="outline" size="sm" className="cursor-pointer bg-transparent text-sm" onClick={() => handleToggleDropdown()}>
                    <Filter className="h-4 w-4 mr-2" />
                    Filter
                  </Button>
                  {isDropdown ?
                    <div className="absolute mt-1 bg-white shadow-md rounded border border-gray-200 w-32 z-10">
                      <button
                      ...
                      <button
                        ...
                        className="block w-full text-left px-3 py-2 text-gray-500 hover:bg-gray-100"
                      >
                        Clear
                      </button>
                    </div>
